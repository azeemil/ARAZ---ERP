    ...
    public class ActiveDirectory
    {
        ...
        public static List<Users> GetAdUsers()
        {
            ...
            try
            {
                ...
                string DomainPath = "LDAP://mdc.retail.local/DC=retail,DC=local";
                ...
                for (var counter = 0; counter < resultCol.Count; counter++)
                {
                    ...
                    try {
                    objSurveyUsers.UserName = (result.Properties["samaccountname"].Count > 0)
                        ? result.Properties["samaccountname"]?[0].ToString()
                        : null;
                        objSurveyUsers.Email = (result.Properties["mail"].Count > 0)
                            ? result.Properties["mail"][0]?.ToString()
                            : null;
                        objSurveyUsers.DisplayName = (result.Properties["displayname"].Count > 0)
                            ? result.Properties["displayname"][0]?.ToString()
                            : null; 
                        objSurveyUsers.FirstName = (result.Properties["givenName"].Count > 0)
                            ? result.Properties["givenName"][0]?.ToString()
                            : null;
                        objSurveyUsers.LastName = (result.Properties["sn"].Count > 0)
                            ? result.Properties["sn"][0]?.ToString()
                            : null;
                        objSurveyUsers.AccountController =
                            !((int)result.Properties["userAccountControl"][0] == 514 ||
                              (int)result.Properties["userAccountControl"][0] == 66050);

                        //objSurveyUsers.AccountController = 
                        //(result.Properties["sn"].Count > 0) && !((int)result.Properties["userAccountControl"][0] == 514 ||
                        //                                                                           (int)result.Properties["userAccountControl"][0] == 66050);
