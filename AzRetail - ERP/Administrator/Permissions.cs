...
namespace ERP.Administrator
{
    ...
    
    {
        ...
        void TreeListNodeGet(TreeListNode node,ref string right)
        {
            ...
            {
                var l = (treenode["PERMISSION"] != null) ? bool.Parse(treenode["PERMISSION"].ToString()) : false;
                if (l)
        ...
        void TreeListLoad()
        {
            ...
            if (permissions.Select("LEVEL=0").Any())
            {
        ...
        void TreeListNodeSet(TreeListNode node,string [] per)
        {

            foreach (TreeListNode treenode in node.Nodes)
            {
                treenode["PERMISSION"] = (Array.FindAll(per, x => x.Contains(treenode["ID"].ToString())).Length > 0);
