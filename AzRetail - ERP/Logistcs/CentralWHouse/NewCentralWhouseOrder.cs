...
{
    public partial class NewCentralWhouseOrder : XtraForm
    {
        public NewCentralWhouseOrder()
        {
            InitializeComponent();
            DataSourceForCategory();
        ...
        public void DataSourceForCategory()
        {
            var dt = Functions.GetSqlServerDataTable(Variables.TigerConnection,
                $@"SELECT  CODE NR,DESCR NAME FROM {Variables.FirmDb}LK_{Variables.FirmNr}_{Variables.FirmPeriod
                    }_PURCHCATEGORY WHERE PARENTREF=0");
            CategoryCmbEdit.Properties.DataSource = dt;
...
DECLARE 
  ...
  IF OBJECT_ID ('TEMPDB..#T_{3}ITEMS_TYPE') IS NOT NULL DROP TABLE #T_{3}ITEMS_TYPE
  

 -----MATERIAL TIPLERI
 SELECT ITEMREF,ABCCODE INTO  #T_{3}ITEMS_TYPE FROM {0}LG_{1}_INVDEF WHERE INVENNO = @AMBAR AND ABCCODE = @ORDERTYPE
 --- AMBAR STOKU
 SELECT  INVENNO SOURCEINDEX,STOCKREF,ROUND(SUM(ONHAND),2) STOK  INTO #T_{3}STOK_AMBAR  FROM {0}LV_{1}_{2}_STINVTOT  
 WHERE INVENNO=@AMBAR AND STOCKREF IN (SELECT ITEMREF FROM #T_{3}ITEMS_TYPE )
...
WHERE STOCKREF IN (SELECT STOCKREF FROM #T_{3}STOK_AMBAR) AND INVENNO = @MARKET  AND STOCKREF IN  (SELECT ITEMREF FROM #T_1ITEMS_TYPE )
 ...
 AMOUNT FLOAT
	...
	SET @COUNTER=0    
		...
		INTO #T_{3}SON
				...
				SELECT T.*,ISNULL(NULLIF(ITMUNITA.CONVFACT2,0),1)/ISNULL(NULLIF(ITMUNITA.CONVFACT1,0),1) ICSAYI, CAST(0 AS FLOAT) RESULT--,
            --    ITMTYPE.ABCCODE,CASE ITMTYPE.ABCCODE WHEN 1 THEN 'Depo' WHEN 2 THEN 'CrossDock' END ABCCODENAME  
                  FROM #T_{3}SON T
          ...
          --LEFT JOIN #T_{3}ITEMS_TYPE ITMTYPE ON ITMTYPE.ITEMREF=ITEMID AND ITMTYPE.ABCCODE IN (1,2)
             ...
             ", Variables.FirmDb, Variables.FirmNr, Variables.FirmPeriod, User.UserId,
                ...
                , SourceWhouseCmbx.Text.Trim(), 2, orderTypeNo);
        }
        ...
        private void Calculate(object sender, EventArgs e)
        {
            ...
            var temp = Functions.GetCheckedComboboxValue(CategoryCmbEdit, CategoryFlt.Checked,'\'');

            if (temp.Length == 0)
            {
                XtraMessageBox.Show("Kateqoriya seçimi yoxdur!", "Diqqət", MessageBoxButtons.OK,
                    MessageBoxIcon.Error);
                return;
            }

            orderTypeNo = orderType.SelectedIndex + 1;
   ...
   VALUES (NULL,0,1,1,'{8}',GETDATE(),{5},{4},{7},{6},GETDATE(),{3}) 
          SELECT SCOPE_IDENTITY()", Variables.FirmDb,
                    Variables.FirmNr, Variables.FirmPeriod, User.UserId, SourceBranchCmbx.Text.Trim(),
                 2  // DestBranchLookUp.Text.Trim()
                    , SourceWhouseCmbx.Text.Trim(),2 //DestWhouseLookUp.Text.Trim()
                     ,orderTypeNo+1
                    )
            };
            try
            ...
            DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs e)
        {
            ...
            if (row != null)
            {
                ...
                if (value > float.Parse(row.Row["SERBESTSTOK"].ToString()) && orderTypeNo==1)
                {
                    gridView1.CancelUpdateCurrentRow();
        ...
        private void CategoryFlt_CheckedChanged(object sender, EventArgs e)
        {
            CategoryCmbEdit.Enabled = CategoryFlt.Checked;
        }
    }
}
