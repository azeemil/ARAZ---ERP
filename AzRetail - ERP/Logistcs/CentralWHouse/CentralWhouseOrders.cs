...
{
    public partial class CentralWhouseOrders : XtraForm
    {
        private readonly MainForm _mainForm;
        private DataTable _desginTable;
        public CentralWhouseOrders()
        {
            InitializeComponent();
        ...
        public CentralWhouseOrders(MainForm mainForm)
        {
            InitializeComponent();
            this._mainForm = Variables.MForm;
            branchCmbx.Properties.DataSource = User.BranchIn;
        ...
        readonly string _query = string.Format(@"               
                    ...
                    CAST((CASE WHEN LTRIM(RTRIM(FIS.EXPLANATION)) IN ('4','5','6') THEN 1 ELSE 0 END) AS BIT) GROUPPED,
                    (CASE WHEN LTRIM(RTRIM(FIS.EXPLANATION)) IN ('2','4') THEN 'Depo' ELSE 'CrossDock' END) ORDERTYPE,
                    ...
                    INNER JOIN L_CAPIWHOUSE WHOUSE2 ON WHOUSE2.NR=FIS.DESTINDEX AND WHOUSE2.FIRMNR={1} WHERE FIS.FICHETYPE=1 AND LTRIM(RTRIM(FIS.EXPLANATION)) IN ('2','3','4','5','6')
        ...
        {
            _mainForm.OpenOrBringToFront(new NewCentralWhouseOrder());
        }
