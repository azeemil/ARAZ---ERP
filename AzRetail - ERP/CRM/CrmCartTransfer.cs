...
namespace ERP.CRM
{
    public partial class CrmCartTransfer : XtraForm
    {
        ...
        private void SetPermissions()
        {
            ...
            {
                SaveBtn.Enabled = 
                    User.UserPermission.Select("LEVEL=0 AND PERMISSION = '5.1.1.4.1'").Any();
               // SetStatus.Enabled = User.UserPermission.Select("LEVEL=0 AND PERMISSION = '5.1.1.4.2'").Any();
                TransferAllData.Enabled =
                    User.UserPermission.Select("LEVEL=0 AND PERMISSION = '5.1.1.4.3'").Any();
...
UPDATE  {Variables.FirmDb}LK_{Variables.FirmNr}_CRMPERSONAL SET SEX={SexRadio.SelectedIndex},
            ...
            string query =
                ...
                BEGIN TRY
		           ...
		           UPDATE inter_puan..INTERBONUS_PUAN SET PUAN=ISNULL(PUAN,0)+{bonusTxt.Text} WHERE Cari_Kod='{Cartnobtnedit.Text}'
          ...
          VALUES(GETDATE(),0,0,'{Cartnobtnedit.Text.Trim()}',{bonusTxt.Text},0,0,0,GETDATE(),'{CartNo.Text} Kartdan transfer') 
                ...
                END CATCH
                    ...
                    TransferAllData.Checked ? cardTransfer : string.Empty,
                    SetStatus.Checked ? setStatusPassiv : string.Empty);
            if (Functions.ExecuteStatement(Variables.ConnectionString, query))
            {
